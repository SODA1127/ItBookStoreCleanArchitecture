...
class TestBookStoreRepository : BookStoreRepository {
    ...
    override suspend fun getBookInfo(isbn13: String): Flow<BookInfoEntity?> =
        withContext(Dispatchers.Main) {
            flow {
                delay(100)
                if (isbn13 == TEST_ISBN13) {
                    emit(
                        Gson().fromJson(BookInfoResponseJson, BookInfoResponse::class.java)
                            .toEntity()
                    )
                } else {
                    emit(null)
    ...
    override suspend fun getBooksInWishList(): Flow<List<BookEntity>> =
        withContext(Dispatchers.Main) {
            flow {
                delay(100)
    ...
    override suspend fun getBookInWishList(isbn13: String): Flow<BookEntity?> =
        withContext(Dispatchers.Main) {
            flow {
                delay(100)
            ...
            val response =
                when {
                    keyword == TEST_KEYWORD && page == null -> {
                        Gson().fromJson(
                            BookSearchResponseJsonFirstPage,
                            BookSearchResultResponse::class.java
                        )
                    }
